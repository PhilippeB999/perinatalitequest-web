/* ============================================================
   PérinatalitéQuest — logique de jeu (gamification)
   Soins à la mère et au nouveau-né — moteur cloné de PédiatrieQuest
   (compétences 27 et 28 du DEP 5325, SASI)

   Chaque app Quest a SES PROPRES clés de stockage, son propre cache de
   service worker et son propre APP_ID Supabase : ne jamais les partager
   d'une app à l'autre.
   ============================================================ */

const STORAGE_KEY = "perinatalitequest_state_v1";
...
function defaultState() {
  return {
    ...
    scores: {},           // { croissance_1: 25, croissance_2: 80, ... } — meilleur score de CHAQUE palier tenté (réussi OU raté) → synchro (pastille rouge côté enseignant)
...
/* Le stade d'avatar (0 à 11) est proportionnel au XP, ramené au XP du
   dernier niveau défini dans data.js. On lit LEVELS plutôt qu'un 3500 en
   dur : ce programme ne compte que 2 compétences, donc un parcours parfait
   rapporte beaucoup moins de XP que PédiatrieQuest et l'avatar resterait
   bloqué à mi-évolution. */
...
/* ------------------ Accès freemium (licence vs gratuit) ------------------
   ...
   de isLicensed(). */
/* ⚠️ Ce programme ne compte que DEUX compétences : la valeur 3 héritée de
   PédiatrieQuest (10 compétences) aurait rendu l'app entièrement gratuite.
   On offre donc la compétence 27 en entier (ses 3 paliers, 36 questions)
   et on verrouille la compétence 28 — soit la moitié du contenu en démo. */
const FREE_COMPETENCIES = 1; // nombre de compétences gratuites sans licence (ajustable)
...
/* Appel à l'action bilingue, rendu via state.lang.
   L'accord se fait au singulier quand une seule compétence est gratuite
   (c'est le cas ici : FREE_COMPETENCIES = 1). */
function premiumLockText() {
  const n = FREE_COMPETENCIES;
  const freeFr = n === 1 ? "Le premier module est gratuit." : `Les ${n} premiers modules sont gratuits.`;
  const freeEn = n === 1 ? "The first module is free." : `The first ${n} modules are free.`;
  const T = {
    fr: {
      badge: "🔒 Licence requise",
      title: "Contenu réservé aux centres licenciés",
      body: `${freeFr} Tu as un code d'accès ? Entre-le ci-dessous pour débloquer tout le programme.`,
      or: "ou",
    ...
    en: {
      ...
      body: `${freeEn} Already have an access code? Enter it below to unlock the full program.`,
      or: "or",
...
function renderOnboarding() {
  ...
  <div class="onboarding">
    ...
    <label class="field-label">${state.lang === 'fr' ? "Ton totem d'étude" : "Your study totem"}</label>
...
const SYNC_KEY = "perinatalitequest_sync_pending";

/* Transforme state.scores ({ croissance_1: 25, croissance_2: 80, ... }) en tableau { module, niveau, score }.
   ...
   L'id de compétence est libre (croissance, m1, c01…) ; on isole le niveau 1-3 en suffixe. */
...
function deviceId() {
  let id = localStorage.getItem("perinatalitequest_device_id");
  if (!id) {
    id = "pnq-" + (crypto.randomUUID ? crypto.randomUUID() : Date.now().toString(36) + Math.random().toString(36).slice(2));
    localStorage.setItem("perinatalitequest_device_id", id);
  }
  return id;
}
